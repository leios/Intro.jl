...
struct PhysicalParticle{T} <: AbstractParticle
    position::T
    velocity::T
    acceleration::T
    mass::Number
...
function gravity(p1::PhysicalParticle, p2::PhysicalParticle; G = 1)
    r2 = sum((p2.position .- p1.position).^2)
    unit_vector = (p2.position .- p1.position) ./ sqrt(r2)
    return (G*p2.mass/(r2+1)) .* unit_vector
end

function gravity(p1::SimpleParticle, p2::SimpleParticle; G = 1)
    r2 = sum((p2.position .- p1.position).^2)
    unit_vector = (p2.position .- p1.position) ./ sqrt(r2)
...
function move_particle(p::SimpleParticle, new_acceleration, dt;
    ...
    return SimpleParticle(Tuple(new_position),
                          Tuple(new_velocity),
...
function run(n_particles::Int, n_steps::Int; dt = 0.01, dim = 2,
    ...
    for i = 1:n_steps
        ...
        if i%plot_steps == 0
            ...
            plt = Plots.scatter(arr[:,1], arr[:,2], xlims=(-2, 2), ylims=(-2,2))
